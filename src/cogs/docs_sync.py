...
If any English documentation files were changed:

```bash
cd /home/ebi/claude-code-discord-bridge
git checkout -b docs-sync/$(date +%Y%m%d-%H%M%S)
git add -A
git commit -m "[docs-sync] Update English documentation

Auto-generated by docs-sync workflow."
git push -u origin HEAD
gh pr create --title "[docs-sync] Update English documentation" --body "Auto-generated documentation sync.

Changes:
- Updated English docs to reflect code changes
...
# Assembled prompts
DOCS_SYNC_PROMPT = _COMMON_HEADER + _PR_STEP_SYNC
...
class DocsSyncCog(commands.Cog):
    ...
    async def _run_docs_sync(
        ...
        if mode == "sync":
            await thread.send("🔄 英語ドキュメントの同期を開始します...")
        else:
